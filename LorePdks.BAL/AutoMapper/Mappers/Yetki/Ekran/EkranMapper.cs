    ...
    public class EkranMapper : MappingProfile
    {
        public EkranMapper()
        {
            CreateMap<t_ekran, EkranDTO>()
                ...
                .ForMember(dest => dest.ustEkranEidDto, opt => opt.MapFrom(src => new EIdDTO { id = Convert.ToInt32(src.UST_EKRAN_ID) }))// dest => src.UST_EKRAN_ID))
            ...
            CreateMap<EkranDTO, t_ekran>()
                ...
                .ForMember(dest => dest.ACIKLAMA, opt => opt.MapFrom(src => src.aciklama))
                .ForMember(dest => dest.UST_EKRAN_ID, opt => opt.MapFrom(src => Convert.ToInt32(src.ustEkranEidDto.id)))
