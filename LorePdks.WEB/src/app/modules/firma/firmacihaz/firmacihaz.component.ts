...
export class FirmaCihazComponent implements OnInit {
  ...
  loadFirmaCihazList(): void {
    ...
    if (this.selectedFirma && this.selectedFirma.eid) {
      ...
      // Tüm firmaların cihazlarını getir 
      this.loading = false;
      this.firmaCihazList = [];
   ...
   */  openLogParserPage(firmaCihaz: FirmaCihazDto): void {
    console.log('openLogParserPage çağrıldı, firmaCihaz:', firmaCihaz);
    
    if (!firmaCihaz || !firmaCihaz.eid) {
      console.error('Cihaz bilgisi eksik!', firmaCihaz);
      this.messageService.add({ severity: 'error', summary: 'Hata', detail: 'Cihaz bilgisi eksik!' });
      return;
    }
    
    // Yeni Log Parser sayfasına yönlendir
    this.router.navigate(['/secure/firma/log-parser', firmaCihaz.eid])
      .then(() => console.log('Navigasyon başarılı'))
